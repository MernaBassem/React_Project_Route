...
// import { AddToWishList, RemoveProductWishList ,GetWishList} from "../../Context/whishListContent";
...
export default function FeatureProduct() {
  ...
  async function addWhichList(id) {
    try {
      ...
      toast.success("Product Added to Favorites");
      setWishlist([...wishlist, id]);
      localStorage.setItem("wishlist", JSON.stringify([...wishlist, id])); // Store wishlist in local storage
    } catch (error) {
      toast.error("Product can not be added");
    }
  }
  ...
  // Effect hook to fetch wishlist details when the component mounts
  useEffect(() => {
    getwishDetail();
